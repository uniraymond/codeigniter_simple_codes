...
<?=script_tag('assets/includes/js/jquery.validate.min.js')?>  
		...
		<div id="content">
			<div class="float-left blog">   
			    <div id="comment_form_body" class="blog">  
					...
					<?=form_open('blog/blog_update/'.$this->uri->segment(3), array('id'=>'edit_blog'));?>  
					<div class="cls">
					...
					<div class="cls"> 
						...
						<?php echo form_textarea('blog', $blog, 'class="com_input comment_body" id="blog"')?> 
					    </div>  
				     </div>
					...
					<?php $blog_public = $blogpublic?True:False; ?>  
					 <?php //echo form_checkbox('blogbepublic', '', $blog_public, 'class="com_input blog_public"')?>     
					<input type="checkbox" name="blogbepublic" class="com_input blog_public" <?php if($blog_public): ?>checked="checked"<?php endif; ?> />
					 <?php echo form_hidden('id', $id)?> 
					  </div>
					
					  <p><input type="submit" value="Update Blog" class="com_sub" /></p>
					  </form> 
					<div class="cls"></div>   
				</div>        
	       </div>
